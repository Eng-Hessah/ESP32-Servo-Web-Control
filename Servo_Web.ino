...
const int redLED    = 18;  // الليد الأحمر
const int greenLED  = 19;  // الليد الأخضر
...
void handleOpen() {
  ...
  digitalWrite(greenLED, HIGH); // عند الفتح يشتغل الأخضر
  digitalWrite(redLED, LOW);    // ويطفي الأحمر
  server.send(200, "text/plain", "Open");
}

void handleClose() {
  myServo.write(CLOSE_ANGLE);
  digitalWrite(redLED, HIGH);   // عند الإغلاق يشتغل الأحمر
  digitalWrite(greenLED, LOW);  // ويطفي الأخضر
...
void setup() {
  ...
  digitalWrite(redLED, HIGH);   // البداية وضع الإغلاق فيشتغل الأحمر
